		...
		if ((lastRoute === opts.shortRoute && newDate - lastRequestTime >= 800) || lastRoute !== opts.shortRoute) { // make sure not to poll more than once per second for the same type of request
			...
			_getCookie('bungled').then(function(token) {
				...
				if (token !== null && token !== false) {
					...
					localStorage.errorMessage = `Error loading user. Please sign out and sign back in to <a href="http://www.bungie.net">linked with bungie.net</a> then click <input type="button" value="reloading the extension" class="reload" />`;
					chrome.cookies.getAll({
						domain: ".bungie.net"
					}, function(cookies) {
						if (chrome.runtime.lastError) {
