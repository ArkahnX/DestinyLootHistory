		...
		database.getMultipleStores(database.allStores).then(function (data) {
			...
			for (var match of data.matches) {
				if (match.activityTypeHashOverride) {
					if (regexMatch.test(DestinyActivityTypeDefinition[match.activityTypeHashOverride].identifier.split("_").join("")) && moment(match.timestamp).isSameOrBefore(maxDate) && moment(match.timestamp).isSameOrAfter(minDate) && match.activityTime > 300) {
